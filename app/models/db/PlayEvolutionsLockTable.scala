...
trait PlayEvolutionsLockTable {
   ...
   *  @param lock Database column lock SqlType(INT), PrimaryKey */
  ...
  class PlayEvolutionsLock(_tableTag: Tag) extends profile.api.Table[PlayEvolutionsLockRow](_tableTag, Some("pets"), "play_evolutions_lock") {
    ...
    /** Database column lock SqlType(INT), PrimaryKey */
