...
trait MedicalTable {
  ...
  case class MedicalRow(dogId: Int, date: Option[java.sql.Date], `type`: String, mDesc: String, doctor: String)
  /** GetResult implicit for fetching MedicalRow objects using plain SQL queries */
  implicit def GetResultMedicalRow(implicit e0: GR[Int], e1: GR[Option[java.sql.Date]], e2: GR[String]): GR[MedicalRow] = GR{
    prs => import prs._
    MedicalRow.tupled((<<[Int], <<[Option[java.sql.Date]], <<[String], <<[String], <<[String]))
  }
